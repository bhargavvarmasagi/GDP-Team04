    ...
    try {
        ...
        orders.forEach(order => {
            ...
            orderRow.innerHTML = `
                        ...
                        if (order.OrderStatus === 'Pending') {
                            return `
                                <li>${product.ProductName} - ${product.quantity}
                                    <span class="availability ${product.StockQuantity > 0 ? 'in-stock' : 'out-of-stock'}">
                ...
                <td><input type="email" class="delivery-partner-email" data-order-id="${order.OrderID}" placeholder="Enter email" value="${order.DeliveryPartnerEmail ? order.DeliveryPartnerEmail : ""}"></td>
                ...
                <td>
                    <select class="order-status-select" data-order-id="${order.OrderID}">
                        ${['Accepted', 'Cancelled', 'Delivery Partner Assigned', 'Pending', 'Processing', 'Shipped', 'Delivered'].map(statusOption => `
...
function attachOrderUpdateListeners() {
    document.querySelectorAll(".update-status-btn").forEach(button => {
        button.addEventListener("click", async function () {
            ...
            const deliveryPartnerEmail = document.querySelector(`.delivery-partner-email[data-order-id="${orderId}"]`).value.trim();
            ...
            try {
                const response = await fetch(`/api/admin/orders/${orderId}/status`, {
                    ...
                    body: JSON.stringify({
                        status: newStatus,
                        deliveryPartnerEmail: deliveryPartnerEmail || null,
